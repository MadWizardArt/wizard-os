...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const image = await prisma.grottoImage.create({ data: { museId: "studio", provider: "reference", contentType, imageData: bytes, byteSize: bytes.length }, select: { id: true } });
    return NextResponse.json({ id: image.id, src: `/api/grotto/images/${image.id}/file`, favorite: false, canonical: false, provider: "reference" }, { status: 201 });
