...
export default function Home() {
  ...
  return <main className="shell"><aside className="sidebar"><div className="brand"><span className="sigil">✦</span><div><h1>Wizard OS</h1><p>Operations Console</p></div></div><nav>{nav.map((item) => <button key={item} onClick={() => item === "The Crucible" ? setView("dashboard") : item === "Money" ? setView("money") : item === "Ventures" ? setView("ventures") : item === "Clients" ? setView("customers") : undefined} className={item === "The Crucible" && view === "dashboard" ? "navItem active" : item === "Money" && view === "money" ? "navItem active" : item === "Ventures" && view === "ventures" ? "navItem active" : item === "Clients" && view === "customers" ? "navItem active" : item === "Projects" && (view === "project" || view === "artwork") ? "navItem active" : "navItem"}>{item}</button>)}</nav><div className="sidebarFoot"><span>System</span><strong>All clear</strong></div></aside><section className="workspace">{view === "dashboard" ? <Dashboard /> : view === "money" ? <MoneyWorkspace /> : view === "ventures" ? <VenturesWorkspace /> : view === "customers" ? <CustomersWorkspace /> : view === "artwork" ? <ArtworkWorkspace /> : <ProjectWorkspace />}</section>
    ...
    {showNewProject && <div className="modalBackdrop" onMouseDown={() => setShowNewProject(false)}><section className="editWindow" onMouseDown={(event) => event.stopPropagation()}><div className="modalHead"><div><p className="eyebrow">New work order</p><h2>Create Project</h2></div><button className="close" onClick={() => setShowNewProject(false)}>×</button></div><div className="formGrid"><Field label="Project title"><input autoFocus value={newProject.title} onChange={(event) => setNewProject({ ...newProject, title: event.target.value })} placeholder="What are you working on?" /></Field><Field label="Project type"><select value={newProject.type} onChange={(event) => chooseProjectType(event.target.value)}><option value="ARTWORK">Original artwork</option><option value="COMMISSION">Commission</option><option value="DIGITAL_PRODUCT">Digital product</option><option value="CONTENT">Content</option><option value="INTERNAL">Internal project</option></select></Field><Field label="Workflow"><select value={newProject.templateId} onChange={(event) => setNewProject({ ...newProject, templateId: event.target.value })}>{templates.map((template) => <option key={template.id} value={template.id}>{template.name}</option>)}</select></Field><Field label="Related contact"><select value={newProject.customerId} onChange={(event) => setNewProject({ ...newProject, customerId: event.target.value })}><option value="">None</option>{customers.map((customer) => <option key={customer.id} value={customer.id}>{customer.name}</option>)}</select></Field><Field label="Value ($)"><input type="number" min="0" value={newProject.value} onChange={(event) => setNewProject({ ...newProject, value: event.target.value })} /></Field><Field label="Due date"><input type="date" value={newProject.dueDate} onChange={(event) => setNewProject({ ...newProject, dueDate: event.target.value })} /></Field><Field label="Next action"><input value={newProject.nextAction} onChange={(event) => setNewProject({ ...newProject, nextAction: event.target.value })} placeholder="Defaults to the first workflow stage" /></Field></div>{formError && <p className="formError">{formError}</p>}<div className="modalFoot"><span>The selected workflow will be copied into this project.</span><button className="primary" disabled={saving || !newProject.title.trim() || !newProject.templateId} onClick={createProject}>{saving ? "Creating…" : "Create work order"}</button></div></section></div>}
