...
const nav = ["The Crucible", "Campaigns", "Projects", "Inventory", "Calendar", "Money", "Ventures", "Clients", "Content", "Automations"];
...
export default function Home() {
  const [view, setView] = useState<"dashboard" | "projects" | "artwork" | "project" | "money" | "ventures" | "customers">("dashboard");
  ...
  const loadProjects = () => {
    return fetch("/api/projects")
      ...
      .catch(() => {
        // Keep the demo projects available until the local database is migrated and seeded.
      });
  };
  useEffect(() => {
    loadProjects();
  ...
  const archiveProject = async () => {
    ...
    setProjectDetail(null); setSelectedProject(null); setView("projects"); loadProjects();
  };
  ...
  const ProjectsWorkspace = () => <>
    ...
    <section className="panel projectListPanel">
      ...
      {projects.length === 0 ? <div className="emptyState"><strong>No projects yet</strong><p>Create a project when there is real work to track.</p><button className="primary" onClick={() => setShowNewProject(true)}>Create first project</button></div> : <div className="projectList">{projects.map((p) => <button className="projectListItem" key={p.id} onClick={() => openProject(p)}>
        <div className="projectListIdentity"><span className={`status ${p.tone}`}>{p.kind}</span><div><strong>{p.title}</strong><span>{p.status} · {p.progress}% complete</span></div></div>
        <div className="projectListMeta"><span>{p.due}</span><small>{p.next ? `Next: ${p.next}` : "No next action"}</small><b aria-hidden="true">›</b></div>
      </button>)}</div>}
    </section>
  </>;
  ...
  return <main className="shell"><aside className="sidebar"><div className="brand"><span className="sigil">✦</span><div><h1>Wizard OS</h1><p>Operations Console</p></div></div><nav>{nav.map((item) => <button key={item} onClick={() => ["Campaigns", "Calendar", "Inventory"].includes(item) ? window.location.assign("/" + item.toLowerCase()) : item === "The Crucible" ? setView("dashboard") : item === "Projects" ? setView("projects") : item === "Money" ? setView("money") : item === "Ventures" ? setView("ventures") : item === "Clients" ? setView("customers") : undefined} className={item === "The Crucible" && view === "dashboard" ? "navItem active" : item === "Money" && view === "money" ? "navItem active" : item === "Ventures" && view === "ventures" ? "navItem active" : item === "Clients" && view === "customers" ? "navItem active" : item === "Projects" && (view === "projects" || view === "project" || view === "artwork") ? "navItem active" : "navItem"}>{item}</button>)}</nav><div className="sidebarBottom"><a className="primary warlockLink" href="/etsy">Warlock</a><div className="sidebarFoot"><span>System</span><strong>All clear</strong></div></div></aside><section className="workspace">{view === "dashboard" ? <Dashboard /> : view === "projects" ? <ProjectsWorkspace /> : view === "money" ? <MoneyWorkspace /> : view === "ventures" ? <VenturesWorkspace /> : view === "customers" ? <CustomersWorkspace /> : view === "artwork" ? <ArtworkWorkspace /> : <ProjectWorkspace />}</section>
