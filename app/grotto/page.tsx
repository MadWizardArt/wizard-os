...
export default function GrottoPage() {
  ...
  const locked = !statusLoading && status && !status.authenticated; const studioCountOptions = status?.studio.maxImages && status.studio.maxImages < 4 ? ["1"] : ["1", "4"]; const title = isMuse ? MUSE_NAMES[space] : "The Atelier";
  return <main className={styles.page} aria-busy={statusLoading}><section className={styles.shell}>
    ...
    {statusLoading ? <p className={styles.connectionNote}>Opening the Grotto…</p> : locked ? <section className={styles.create}><div className={styles.createInner}><form onSubmit={unlock} className={styles.unlock}><label className={styles.studioField}><span>Artist key</span><input type="password" autoComplete="current-password" value={accessKey} onChange={(event) => setAccessKey(event.target.value)} required/></label><button className={styles.generate}>Unlock the Grotto</button><p className={styles.connectionNote}>{note}</p></form></div></section> : <>
      ...
      : <section className={styles.museGallerySection}><p className={styles.galleryIntro}>Choose an image for the Atelier or add an approved reference.</p><div className={styles.galleryToolbar}><div className={styles.galleryActions}><label className={styles.galleryUpload}>+ Add image<input type="file" accept="image/png,image/jpeg,image/webp" onChange={(event) => { void uploadImage(event.target.files?.[0]); event.target.value = ""; }}/></label><button type="button" onClick={() => { setSelecting((value) => !value); setSelectedIds([]); }}>{selecting ? "Cancel selection" : "Select images"}</button>{selecting && <button type="button" className={styles.deleteSelection} disabled={!selectedIds.length} onClick={() => void deleteSelected()}>Delete selected ({selectedIds.length})</button>}</div></div><div className={styles.gallery}>{items.map((item, index) => <button className={`${styles.tile} ${selectedIds.includes(item.id) ? styles.tileSelected : ""}`} type="button" key={item.id} onClick={() => selecting && item.private && !item.canonical ? toggleSelected(item.id) : !selecting ? setActiveIndex(index) : undefined}><img src={item.src} alt={item.alt} loading="lazy"/>{selecting && item.private && !item.canonical && <span className={styles.selectMark}>{selectedIds.includes(item.id) ? "✓" : ""}</span>}{item.canonical && <span className={styles.canon}>Canon</span>}{item.favorite && !item.canonical && <span className={styles.canon}>♥</span>}</button>)}</div><div ref={loadMoreSentinel} className={styles.scrollSentinel} aria-hidden="true"/>{loadingMore && <p className={styles.scrollStatus}>Loading more…</p>}{note && <p className={styles.connectionNote}>{note}</p>}</section>}
    </>}
  </section>{current && <div ref={viewer} className={styles.lightbox} role="dialog" aria-modal="true" aria-label="Gallery viewer" onTouchStart={(event) => { swipeStart.current = event.touches[0]?.clientX ?? null; }} onTouchEnd={(event) => { if (swipeStart.current === null) return; const distance = (event.changedTouches[0]?.clientX ?? swipeStart.current) - swipeStart.current; swipeStart.current = null; if (Math.abs(distance) >= 48) void browseViewer(distance > 0 ? -1 : 1); }}><button className={styles.close} type="button" onClick={() => setActiveIndex(null)} aria-label="Close">×</button><button className={styles.prev} type="button" disabled={activeIndex === 0} onClick={() => void browseViewer(-1)} aria-label="Previous image">‹</button><img className={styles.lightboxImage} src={current.src} alt={current.alt}/><button className={styles.next} type="button" disabled={activeIndex === items.length - 1 && !hasNext} onClick={() => void browseViewer(1)} aria-label="Next image">›</button>{itemMuseId(current) && <div className={styles.viewerProvenance}>{MUSE_NAMES[itemMuseId(current)!]} Gallery{current.canonical ? " · Canonical reference" : ""}</div>}<div className={styles.viewerActions}>{current.private && <button type="button" onClick={() => void toggleFavorite()}>{current.favorite ? "Favorited" : "Favorite"}</button>}{current.prompt && <button type="button" onClick={() => void copyPrompt()}>Copy prompt</button>}{ROLES.map((role) => <button type="button" key={role} onClick={() => putInTray(current, role, role === "primary")}>{role === "primary" ? "Remix in Atelier" : `Use as ${ROLE_LABELS[role]}`}</button>)}<a href={current.src} download={`grotto-${current.id}`}>Download</a>{current.private && <button type="button" onClick={() => void deleteCurrent()}>Delete</button>}</div></div>}</main>;
}
