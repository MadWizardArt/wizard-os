...
export default function GrottoPage() {
  ...
  const selectSpace = async (next: Space, afterMove = false) => { if (generating || uploading || (managing && !afterMove)) return false; galleryRequest.current += 1; loadMoreBusy.current = false; setLoadingMore(false); setGalleryPage(0); setHasNext(false); setSpace(next); setItems([]); setActiveIndex(null); setSelecting(false); setSelectedIds([]); setMoveTarget(""); setViewerTarget(""); setHeaderSrc(null); setNote(""); if (status?.authenticated) try { await Promise.all([loadGallery(next), ...(MUSES.includes(next as MuseId) ? [loadHeader(next as MuseId)] : [])]); return true; } catch (error) { setNote(error instanceof Error ? error.message : "Gallery could not be opened."); return false; } return false; };
  useEffect(() => {
  ...
  const moveImages = async (ids: string[], destination: "studio" | MuseId | "") => {
    ...
    try {
      ...
      const opened = await selectSpace(destination, true);
      if (opened) {
        setNote(`Moved ${payload.moved} image${payload.moved === 1 ? "" : "s"} to ${destination === "studio" ? "the Atelier" : MUSE_NAMES[destination]}.`);
        window.requestAnimationFrame(() => document.getElementById("grotto-gallery")?.scrollIntoView({ behavior: "smooth", block: "start" }));
      }
