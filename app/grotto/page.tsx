...
type StudioInput = { environmentId: EnvironmentId; prompt: string; negativePrompt: string; format: StudioFormat; quantity: 1 | 4; referenceId?: string; strength?: number; loras?: LoraSelection[] };
...
export default function GrottoPage() {
  const viewer = useRef<HTMLDivElement>(null); const galleryRequest = useRef(0); const loadMoreSentinel = useRef<HTMLDivElement>(null); const loadMoreBusy = useRef(false);
  ...
  const current = activeIndex === null ? null : items[activeIndex]; const primary = tray.find((item) => item.role === "primary") ?? null; const isMuse = space !== "studio";
  ...
  useEffect(() => { let live = true; void (async () => { try { const payload = await readJson(await fetch("/api/grotto/status", { cache: "no-store" })) as GrottoStatus; if (!live) return; setStatus(payload); setStudioNegative(payload.studio.defaultNegative || ""); setStudioEnvironment(preferredEnvironment(payload.studio)); if (payload.authenticated) await loadGallery("studio"); } catch (error) { if (live) setNote(error instanceof Error ? error.message : "The Grotto could not be opened."); } finally { if (live) setStatusLoading(false); } })(); return () => { live = false; }; }, []);
  const selectSpace = async (next: Space) => { if (generating || uploading) return; loadMoreBusy.current = false; setLoadingMore(false); setSpace(next); setItems([]); setActiveIndex(null); setNote(""); if (status?.authenticated) try { await loadGallery(next); } catch (error) { setNote(error instanceof Error ? error.message : "Gallery could not be opened."); } };
  useEffect(() => {
  ...
  return <main className={styles.page} aria-busy={statusLoading}><section className={styles.shell}>
    ...
    <section className={`${styles.hero} ${!isMuse ? styles.studioHero : ""}`} aria-label={`${title} in the Grotto`}>{isMuse && <img className={styles.heroImage} src={`/museum/${space}.webp`} alt=""/>}<div className={styles.heroVeil}/><div className={styles.heroCopy}><small>The Grotto</small><h1>{title}</h1>{isMuse && <p>Approved references and visual studies. The gallery supports the Muse’s canon; it does not replace it.</p>}</div></section>
