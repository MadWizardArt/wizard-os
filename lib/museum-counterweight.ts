...
export function decodeCounterweightPacket(notes: string | null): StoredCounterweightPacket | null {
  ...
  try {
    ...
    const primaryKernel = getMuseMindKernel(primaryMuseId);
    const counterweightKernel = getMuseMindKernel(counterweightMuseId);
    if (!primaryKernel || !counterweightKernel) return null;
